...
```sh
chmod u+x ./citrea-v0.7.2-osx-arm64
```

```sh
chmod u+x ./citrea-v0.7.2-linux-amd64
```
...
Mac:
```sh
./citrea-v0.7.2-osx-arm64 --da-layer bitcoin --rollup-config-path ./rollup_config.toml --genesis-paths ./genesis --network testnet
```

Linux:
```sh
./citrea-v0.7.2-linux-amd64 --da-layer bitcoin --rollup-config-path ./rollup_config.toml --genesis-paths ./genesis --network testnet
```
